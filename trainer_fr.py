...
""" train model """
def train_epoch_fr(config, epoch, model_transformer, model_backbone, save_output, criterion, optimizer, scheduler, train_loader):
    losses = []
    ...
    return np.mean(losses), rho_s, rho_p, pred_epoch


""" validation """
def test_epoch_fr(config, epoch, model_transformer, model_backbone, save_output, criterion, test_loader):
    with torch.no_grad():
        losses = []
